package com.dgsl.restapi.util;

public class Constants {

	public static final String CONFIG_FILE = System.getProperty("user.dir")
			+ "\\src\\main\\java\\com\\dgsl\\restapi\\config\\config.properties";

	public static final String CHROME_DRIVER = System.getProperty("user.dir") + "\\BrowserDriver\\chromedriver2.8.exe";

	public static final String IE_DRIVER = System.getProperty("user.dir") + "\\BrowserDriver\\IEDriverServer.exe";

	public static final String GECKO_DRIVER = System.getProperty("user.dir")
			+ "\\BrowserDriver\\geckodriver win32 0.20.exe";

	public static final String EXTENT_CONFIG = System.getProperty("user.dir")
			+ "\\FreeCRM Cucumber\\src\\main\\java\\com\\dgsl\\freecrm\\config\\extent-config.xml";

	public static final String JSON_FILE = System.getProperty("user.dir")
			+ "\\src\\main\\java\\com\\dgsl\\restapi\\data\\users.json";

}
